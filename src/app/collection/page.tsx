...
export default async function CollectionPage() {
    ...
    const rawAntiques = response.data.map((item: RawAntique) => ({
        ...
        : "",
        AdditionalPhotoURLs: item.AdditionalPhotos.map(
        (photo : AdditionalPhoto) => `${process.env.STRAPI_URL}${photo.formats?.thumbnail?.url ?? ""}`
        ) ?? [],
    ...
    return (
        ...
        // Collection
        <div
            className="grid grid-cols-2 gap-x-4 px-6 py-16 lg:grid-cols-3"
        >
